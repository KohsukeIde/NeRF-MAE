...
SAVE_NAME="${SAVE_NAME:-${DATASET_NAME}_scratch_samepath_fcos${FCOS_NUM_EPOCHS}}"
...
[[ -n "${BEST_CHECKPOINT}" ]] || {
  echo "[error] no fair-scratch FCOS checkpoint found in ${SAVE_PATH}" >&2
  exit 1
}
